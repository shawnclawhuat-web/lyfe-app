...
const QUESTIONS: ExamQuestion[] = [
  { id: 'q1', paper_id: 'p1', question_number: 1, question_text: 'What is 1+1?', options: ['1', '2', '3', '4'], correct_answer: '2', explanation: 'Basic math' },
  { id: 'q2', paper_id: 'p1', question_number: 2, question_text: 'What is 2+2?', options: ['3', '4', '5', '6'], correct_answer: '4', explanation: null },
  { id: 'q3', paper_id: 'p1', question_number: 3, question_text: 'What is 3+3?', options: ['5', '6', '7', '8'], correct_answer: '6', explanation: null },
  { id: 'q4', paper_id: 'p1', question_number: 4, question_text: 'What is 4+4?', options: ['7', '8', '9', '10'], correct_answer: '8', explanation: null },
  { id: 'q5', paper_id: 'p1', question_number: 5, question_text: 'What is 5+5?', options: ['9', '10', '11', '12'], correct_answer: '10', explanation: null },
];
...
describe('submitExamAttempt', () => {
  ...
  it('calculates passing score at 70% threshold', async () => {
    ...
    const result = await submitExamAttempt(
      {
        userId: 'u1', paperId: 'p1', questions: QUESTIONS,
        answers: { q1: '2', q2: '4', q3: '6', q4: '8', q5: 'wrong' },
        status: 'submitted', startedAt: Date.now() - 60000,
      },
      'M9',
    );
    ...
    const result = await submitExamAttempt(
      {
        ...
        status: 'auto_submitted', startedAt: Date.now() - 300000,
      },
      'M9',
    );
  ...
  it('returns error when attempt insert fails', async () => {
    ...
    const result = await submitExamAttempt(
      {
        userId: 'u1', paperId: 'p1', questions: QUESTIONS,
        answers: {}, status: 'submitted', startedAt: Date.now(),
      },
      'M9',
    );
  ...
  it('still returns result when answer insert fails (logs only)', async () => {
    const attemptsChain = mockSupa.__getChain('exam_attempts');
    ...
    const result = await submitExamAttempt(
      {
        userId: 'u1', paperId: 'p1', questions: QUESTIONS,
        answers: { q1: '2' }, status: 'submitted', startedAt: Date.now(),
      },
      'M9',
    );

    // Should still return result despite answer insert failure
    expect(result.data).not.toBeNull();
    expect(result.data?.id).toBe('attempt-5');
...
describe('fetchExamResult', () => {
  it('returns result with questions and answers joined', async () => {
    ...
    mockResolve(attemptsChain, {
      data: {
        id: 'attempt-1', paper_id: 'p1', score: 4, total_questions: 5,
        percentage: 80, passed: true, status: 'submitted',
      },
      error: null,
    });
    ...
    mockResolve(answersChain, {
      data: [
        {
          selected_answer: '2', is_correct: true,
          exam_questions: QUESTIONS[0],
        },
        {
          selected_answer: 'wrong', is_correct: false,
  ...
  it('returns error when answers query fails', async () => {
    const attemptsChain = mockSupa.__getChain('exam_attempts');
    mockResolve(attemptsChain, {
      data: { id: 'attempt-1', paper_id: 'p1', score: 0, total_questions: 5, percentage: 0, passed: false, status: 'submitted' },
      error: null,
    });
