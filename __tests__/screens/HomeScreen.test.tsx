...
beforeEach(() => {
  ...
  (useTheme as jest.Mock).mockReturnValue({
    colors: Colors.light, isDark: false, mode: 'light', resolved: 'light', setMode: jest.fn(),
  });
  ...
  (fetchLeadStats as jest.Mock).mockResolvedValue({
    data: {
      totalLeads: 10, newThisWeek: 2, conversionRate: 25, activeFollowUps: 3,
      pipeline: [
        { status: 'new', count: 3 }, { status: 'contacted', count: 2 },
        { status: 'qualified', count: 2 }, { status: 'proposed', count: 1 },
        { status: 'won', count: 1 }, { status: 'lost', count: 1 },
      ],
    },
    error: null,
  });
  ...
  (fetchManagerDashboardStats as jest.Mock).mockResolvedValue({
    data: { activeCandidates: 5, agentsManaged: 3 }, error: null,
  });
...
describe('HomeScreen', () => {
  ...
  it('shows agent quick actions', async () => {
    ...
    await waitFor(() => {
      expect(getByText('Add Lead')).toBeTruthy();
      expect(getByText('All Leads')).toBeTruthy();
      expect(getByText('Follow-ups')).toBeTruthy();
  ...
  it('shows PA events section', async () => {
    ...
    (fetchUpcomingEvents as jest.Mock).mockResolvedValue({
      data: [{
        id: 'evt-1', title: 'Team Standup', event_type: 'team_meeting',
        event_date: '2026-03-15', start_time: '09:00', end_time: '10:00',
